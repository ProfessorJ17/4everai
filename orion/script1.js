...
function initializeAppUI() {
    // This function now contains all the setup that should run AFTER login.
    ...
    if (savedThemeMode === 'light') {
        ...
        document.getElementById('theme-toggle-btn').querySelector('i').className = 'fas fa-sun';
    }

    const savedColorTheme = localStorage.getItem('orion_color_theme') || 'orion';
    document.body.classList.add(savedColorTheme + '-theme');
    const themeOption = document.querySelector(`#theme-dropdown a[data-value="${savedColorTheme}"]`);
    document.getElementById('selected-theme').textContent = themeOption ? themeOption.textContent : savedColorTheme.toUpperCase();
    glowManager.updateTheme(savedColorTheme);

    loadCustomPersonas();
    currentPersonaId = localStorage.getItem('orion_selectedPersonaId') || 'caimeo';
    populateModels();
    populatePersonas();
    ...
    try {
        ...
        if (creditDocSnap.exists()) {
            ...
            if (shouldResetCredits(data.lastReset)) {
                // Time to reset credits (7 PM has passed)
                ...
                userCredits = { count: data.count, lastReset: new Date(data.lastReset.seconds * 1000) };
            }
        } else {
            // First time user
...
async function decrementCredits() {
    ...
    if (!user || subscriptionDetails.status === 'Active') return;
    ...
    const creditDocRef = doc(db, config.USER_CREDITS_COLLECTION_NAME, user.uid);
    try {
        await updateDoc(creditDocRef, { count: increment(-1) });
        
        // Record message usage timestamp
        const messageUsageRef = collection(db, config.USER_MESSAGE_USAGE_COLLECTION_NAME);
        await addDoc(messageUsageRef, {
            userId: user.uid,
            timestamp: serverTimestamp(),
            creditUsed: true
        });
    } catch (error) {
        console.error("Failed to decrement credits in Firestore:", error);
    }
}
...
function setupUIEventListeners() {
    ...
    subscriptionBtn.addEventListener('click', async () => {
        ...
        // Fetch message usage history for inactive users
            ...
            try {
                ...
                if (!querySnapshot.empty) {
                    ...
                    contentHTML += `
                        ...
                        <div class="message-usage-section">
                            <h4 style="margin-bottom: 0.75rem; color: var(--text-color);">Credits Used:</h4>
                            <p style="color: var(--placeholder-color); font-style: italic;">No messages sent yet.</p>
                        </div>
                    `;
                }
            } catch (error) {
                ...
                contentHTML += `
                    ...
                    <div class="message-usage-section">
                        <h4 style="margin-bottom: 0.75rem; color: var(--text-color);">Credits Used:</h4>
                        <p style="color: var(--placeholder-color); font-style: italic;">Unable to load message history.</p>
                    </div>
                `;
            }
        }
    ...
    setupDropdown('theme-btn', 'theme-dropdown', 'selected-theme');
        ...
        document.querySelectorAll('.custom-dropdown-menu').forEach(dropdown => {
            let buttonId;
            if (dropdown.id === 'model-selector') {
                buttonId = 'chat-model-btn';
            } else if (dropdown.id === 'personas-content-container') {
                buttonId = 'persona-btn';
            } else if (dropdown.id === 'theme-dropdown') {
                buttonId = 'theme-btn';
            }
